...
class TestLLMManagerTools:
    ...
    def test_call_with_tools_without_litellm_returns_mock(self, llm_manager, conversation_history, sample_tools):
    ...
    @pytest.mark.asyncio
    async def test_call_with_tools_success_no_tool_calls(self, llm_manager, conversation_history, sample_tools):
        ...
        with patch('managers.llm.llm_manager.LITELLM_AVAILABLE', True):
            with patch('managers.llm.llm_manager.acompletion', new_callable=AsyncMock) as mock_completion:
    ...
    @pytest.mark.asyncio
    async def test_call_with_tools_with_tool_calls(self, llm_manager, conversation_history, sample_tools):
        ...
        with patch('managers.llm.llm_manager.LITELLM_AVAILABLE', True):
            with patch('managers.llm.llm_manager.acompletion', new_callable=AsyncMock) as mock_completion:
                ...
                assert tool_call["arguments"] == {"expression": "2+2"}  # Should be parsed JSON
    
    @pytest.mark.asyncio
    async def test_tool_arguments_json_parsing_failure(self, llm_manager, conversation_history, sample_tools):

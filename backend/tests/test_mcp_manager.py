...
async def test_mcp_manager_initialization(mcp_manager, mock_config_manager):
    ...
    def mock_from_pydantic(pydantic_obj):
        if hasattr(pydantic_obj, "transport") and pydantic_obj.transport == "stdio":
            ...
            hasattr(pydantic_obj, "config") and pydantic_obj.config.transport == "stdio"
        ):
            return mock_config_stdio
        else:
    ...
    with (
        ...
        patch.object(
            mcp_manager, "_initialize_clients", new_callable=AsyncMock
        ) as mock_init_clients,
        patch.object(
            mcp_manager, "_load_server_configs", new_callable=AsyncMock
...
@pytest.mark.asyncio
async def test_mcp_manager_initialize_clients(
    ...
    with (
        ...
        patch.object(
            mcp_manager, "_create_stdio_client", return_value=mock_fastmcp_client
        ) as mock_create_stdio,
        patch.object(
            mcp_manager, "_create_http_client", return_value=mock_fastmcp_client
        ) as mock_create_http,
    ):
...
async def test_mcp_manager_discover_tools(
    ...
    assert (
        len(mcp_manager.tool_registry._tools) == 4
    )  # Corrected attribute: 2 tools from each server
...
@pytest.mark.asyncio
async def test_mcp_manager_get_available_servers(
    mcp_manager, mock_config_manager, mock_fastmcp_client
):
...
async def test_mcp_manager_get_authorized_servers(mcp_manager, mock_config_manager):
    ...
    server3 = MCPServer(
        name="public_server", config=MCPServerConfig(groups=[])
...
@pytest.mark.asyncio
async def test_mcp_manager_get_available_tools(
    mcp_manager, mock_config_manager, mock_fastmcp_client
):
...
@pytest.mark.asyncio
async def test_mcp_manager_get_tools_for_servers(
    mcp_manager, mock_config_manager, mock_fastmcp_client
):
    ...
    tools_for_both = mcp_manager.get_tools_for_servers(
...
@pytest.mark.asyncio
async def test_mcp_manager_get_tool_by_name(
    mcp_manager, mock_config_manager, mock_fastmcp_client
):

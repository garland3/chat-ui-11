...
class TestToolCallerAuthorization:
    ...
    def test_authorization_blocks_unauthorized_user_from_restricted_servers(
        self, tool_caller
    ):
        ...
        tool_caller.mcp_manager.get_tools_for_servers.assert_called_once_with(
            ["server3"]
        )
    ...
    def test_tool_filtering_works_with_authorization(self, tool_caller):
        ...
        # Mock tool objects with to_openai_schema method
        mock_tool1 = Mock()
        mock_tool1.to_openai_schema.return_value = {
            "function": {"name": "server2_allowed_tool"}
        }
        mock_tool2 = Mock()
        mock_tool2.to_openai_schema.return_value = {
            "function": {"name": "server2_another_tool"}
        }

        tool_caller.mcp_manager.get_tools_for_servers.return_value = [
            mock_tool1,
            mock_tool2,
        ]
        ...
        result = tool_caller.get_authorized_tools_for_user(
            username="test_user",
            selected_tool_map={
                "server2": ["allowed_tool"]
            },  # Only request one specific tool on authorized server
            is_user_in_group=test_user_check,
        )

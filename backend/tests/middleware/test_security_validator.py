...
class TestSecurityValidator:
    ...
    @pytest.mark.asyncio
    async def test_validate_authentication_debug_mode_with_header(self, mock_app_factory):
        """Test authentication validation in debug mode with header."""
        mock_config_manager = MockConfigManager(debug_mode=True, test_user="debug@test.com")
        mock_app_factory.get_config_manager.return_value = mock_config_manager
        
        validator = SecurityValidator()
        headers = {"x-user-email": "user@example.com"}
        
        is_valid, user_email, error = await validator.validate_authentication(headers, debug_mode=True)
    ...
    @patch('middleware.security_validator.app_factory')
    @pytest.mark.asyncio
    async def test_validate_authentication_debug_mode_without_header(self, mock_app_factory):
    ...
    async def test_validate_authentication_production_mode_valid(self):
        ...
        is_valid, user_email, error = await validator.validate_authentication(headers, debug_mode=False)
    ...
    def test_validate_rate_limit_first_request(self, mock_app_factory):
        ...
        mock_config_manager = MockConfigManager(rate_limit_rpm=10, rate_limit_window_seconds=60)
        mock_app_factory.get_config_manager.return_value = mock_config_manager
    ...
    def test_validate_rate_limit_exceeded(self, mock_time, mock_app_factory):
        ...
        mock_config_manager = MockConfigManager(rate_limit_rpm=2, rate_limit_window_seconds=60)
        mock_app_factory.get_config_manager.return_value = mock_config_manager
...
class TestWebSocketSecurity:
    ...
    @patch('middleware.security_validator.app_factory')
    @pytest.mark.asyncio
    async def test_validate_websocket_security_success(self, mock_app_factory, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_validate_websocket_security_rate_limit_failure(self, mock_validator, mock_app_factory, mock_websocket):
        ...
        mock_validator.validate_authentication = AsyncMock(return_value=(True, "user@example.com", None))

...
class TestRateLimitMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_request_allowed(self, mock_validator, mock_app, mock_request, mock_call_next):
        ...
        mock_validator.validate_rate_limit.assert_called_once_with("127.0.0.1", "/api/test")

    @patch('middleware.rate_limit_middleware.security_validator')
    @pytest.mark.asyncio
    async def test_request_rate_limited(self, mock_validator, mock_app, mock_request, mock_call_next):
    ...
    async def test_request_no_client(self, mock_validator, mock_app, mock_call_next):
        ...
        assert response.status_code == 200
        mock_validator.validate_rate_limit.assert_called_once_with("unknown", "/api/test")

    @patch('middleware.rate_limit_middleware.security_validator')
    @pytest.mark.asyncio
    async def test_request_no_host_attribute(self, mock_validator, mock_app, mock_call_next):

...
class TestAuthMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_production_mode_with_header(self, mock_app, mock_request, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_production_mode_without_header(self, mock_app, mock_request, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_production_mode_with_empty_header(self, mock_app, mock_request, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_production_mode_with_whitespace_header(self, mock_app, mock_request, mock_call_next):
    ...
    async def test_debug_mode_without_header_uses_test_user(self, mock_app_factory_module, mock_app, mock_request, mock_call_next):
        """Test debug mode without header uses test user from config."""
        mock_factory = MockAppFactory("debug@test.com")
        mock_app_factory_module.get_config_manager.return_value = mock_factory.config_manager
    ...
    @pytest.mark.asyncio
    async def test_header_extraction_strips_whitespace(self, mock_app, mock_request, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_different_api_paths_require_auth(self, mock_app, mock_request, mock_call_next):
        ...
        for path in test_paths:
            ...
            assert isinstance(response, RedirectResponse), f"Path {path} should require auth"
            assert response.status_code == 302
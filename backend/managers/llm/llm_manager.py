...
class LLMManager:
    ...
    async def call_with_tools(self, model_name: str, conversation_history: ConversationHistory, tools: List[Dict[str, Any]], tool_choice: str = "auto", temperature: float = 0.7) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"Calling LLM with tools: {litellm_model} with {len(messages)} messages and {len(tools)} tools")
            ...
            logger.info(f"LLM response received: {len(content)} characters, {len(tool_calls)} tool calls")
            return {
                "content": content,

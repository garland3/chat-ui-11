...
class MCPManager:
    ...
    async def _initialize_clients(self) -> None:
        ...
        # Initialize clients in parallel
        tasks = [
            self._initialize_client(server)
    ...
    async def _initialize_client(self, server: MCPServer) -> Optional[Client]:
        ...
        try:
            ...
            if config.transport == 'stdio':
                ...
                logger.error(f"Unsupported transport '{config.transport}' for server {server.name}")
                return None
    ...
    def _create_stdio_client(self, server: MCPServer) -> Optional[Client]:
        ...
        if config.command:
            ...
            if server_path.exists():
                ...
                logger.error(f"No command specified and default server not found: {server_path}")
                return None
    ...
    async def _discover_tools_for_server(self, server_name: str, client: Client) -> List[MCPTool]:
            ...
            async with client:
                ...
                for raw_tool in raw_tools:
                    tool = MCPTool(
                        ...
                        tags=getattr(getattr(raw_tool, 'meta', {}), 'get', lambda x, d: d)('tags', set())
                    )
                    tools.append(tool)
                
                logger.debug(f"Found {len(tools)} tools in {server_name}: {[t.name for t in tools]}")

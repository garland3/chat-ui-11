...
async def validate_websocket_security(
    ...
    try:
        ...
        if getattr(config_manager.app_settings, 'security_ws_origin_check_enabled', False):
            allowed = getattr(config_manager.app_settings, 'security_ws_allowed_origins', []) or []
            if not origin or origin not in allowed:
                return False, None, "Invalid WebSocket Origin"
        
    # Validate authentication
        auth_valid, user_email, auth_error = await security_validator.validate_authentication(
            headers, debug_mode
        )
        ...
        if not rate_valid:
            return False, None, f"Rate limit exceeded. Retry after {retry_after} seconds"
